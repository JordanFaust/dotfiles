...
local function progressbar_widget(config)
    ...
    -- register component
    registry.add("sidebar::cpu::progressbar", widget)

    -- setup signals
    widget:connect_signal("update::value", function(progressbar, conf)
        widget.value = conf.value
    end)
...
local function start_watch()
    awful.widget.watch(command, 5, function(_, stdout)
        local cpu_idle = stdout
        cpu_idle = string.gsub(cpu_idle, '^%s*(.-)%s*$', '%1')
        registry.emit("sidebar::cpu::progressbar", "update::value", { value = tonumber(cpu_idle) })
    end)
end

function cpu.widget(default, overrides)
    ...
    registry.add("sidebar::cpu", widget)
    start_watch()
